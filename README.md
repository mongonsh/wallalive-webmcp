...
WallAlive is a child-authored storytelling scaffold, not an automatic homework system. The complete learning loop is **Imagine → Sequence → Perform → Reflect**: a learner creates a drawing, the agent turns verified character abilities into a beginning–middle–end plan, the child approves and performs it, then retells or revises the story. The private Story Passport records completed beats, shared activity, the learner's retell, and one chosen revision; `inspect_learning_progress` lets the agent use that structured evidence for its next scaffold without grading the child or receiving pixels. That loop can support narrative sequencing, oral language and emotion vocabulary, collaboration, creative confidence, and practical AI consent literacy.
...
| `inspect_learning_progress` | Read | Returns planned/completed beats, collaborative activity, the learner's reflection, and an evidence-based next scaffold without pixels or grades. |
