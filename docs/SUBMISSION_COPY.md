...
WallAlive lets friends join by guest username, paint one vector wall, combine separate figures into a cast, paint directly on the live 3D model, and play cooperative activities in original 3D worlds. Seven local ONNX graphs isolate selected figures, preserve artwork, decode variable skeletons, and lock uncertain movement. The private tier is honestly labeled a spatial puppet; optional AniGen generates full unseen surfaces. Nineteen goal-level WebMCP tools inspect the room, per-figure reconstruction readiness, verified rigs, 3D paint state, and private Story Passport; request visible human repair; stage a child-controlled paint adventure; adapt the next challenge; prepare invites; stage capability-safe stories; activate real 3D quest objects; recommend collaborative products; and stage a creator-credited Shopify handoff. Camera, pixels, painting, correction, approval, publishing, and purchase stay human-controlled.

## Judging case

### 1. Use of WebMCP

WebMCP is the creative control plane, not decorative integration. Nineteen imperative tools implement six shared-authority loops: inspect readiness → request visible human repair → re-inspect; inspect room → prepare invite → co-create; inspect 3D paint state → stage palette adventure → child paints; inspect rigs/objects → stage → human-approved performance; inspect Story Passport → stage the next evidence-based scaffold; and inspect artwork evidence → recommend products → stage Creator Drop → adult and contributor-approved export. The agent coordinates structured work while the person retains sensor, painting, correction, sharing, publishing, commerce, and final-performance authority.
...
WallAlive turns a learner's own artifact into a concrete cycle: **Imagine → Sequence → Perform → Reflect**. The agent can scaffold a beginning, middle, and ending; assign cooperative roles; and select only movements the verified rigs support. The child approves the plan, performs it, then retells or revises it. Potential benefits include narrative sequencing, oral language and emotion vocabulary, collaboration, creative confidence, and practical AI consent literacy at home, in classrooms, or in museum workshops. These are intended benefits—not measured learning gains. WallAlive has not completed a classroom efficacy study and is not an assessment or therapy tool; a teacher-led pilot is the next evidence step.
...
Press **Play Judge Demo**. This clearly labeled **Guided Demo** loads the exact supplied drawing and its precomputed full neural, graph-skinned GLB, then runs the same canonical creation, placement, bone animation, story, and provenance paths without camera, WebXR hardware, or public GPU quota. Its activity is attributed to `WALLALIVE`, never to `BROWSER AGENT`; use the primary prompt above to prove real WebMCP agent planning.
