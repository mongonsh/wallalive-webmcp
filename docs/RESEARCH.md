...
- Register ten goal-level tools with `document.modelContext.registerTool()` in the top-level page, including spatial cinematics plus evidence-based Creator Drop recommendation, staging, and inspection. Shopify publishing and purchase remain outside WallAlive's tool surface.
