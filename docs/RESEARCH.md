...
On the untouched official test set, calibrated body/limb IoU is 0.901 body, 0.603 arm, 0.540 hand, 0.696 leg, and 0.656 foot. The face ensemble improves every class over the v3 crop: eye 0.6419→0.6578, cheek/facial accessory 0.1652→0.1962, mouth 0.5688→0.5753, and ear 0.4494→0.4791. Macro face IoU rises from 0.4563 to 0.4771 (+4.6% relative). Official-test part-presence F1 is 0.9629 eye, 0.4371 cheek/accessory, 0.9339 mouth, and 0.6909 ear. The heterogeneous accessory label remains the weakest class, so that limitation is reported rather than hidden behind a generic “SAM-like” claim.

The ensemble decodes up to six face instances and ten appendages per class. Predictions from the enlarged face and whole-character passes are merged only when their centers and measured extents indicate the same instance. The older v2/v1 checkpoints stay off the mobile critical path and download only if the primary stack misses an eye, cheek/facial mark, mouth, arm, or leg anchor; they may restore a missing bilateral mate but cannot flood the rig with duplicates.
